...
msg="Hello Patient \n Book your appointment or Consult a doctor \n You have been detected with moderate retinal disease"
def sms_send():
...
root.title("Retinal Disease Detection")
...
image2 =Image.open('a1.webp')
image2 =image2.resize((1300,700), Image.LANCZOS)
...
#
# lbl = tk.Label(root, text="Retinal Disease Detection", font=('times', 35,' bold '), height=1, width=60,bg="maroon",fg="white")
# lbl.place(x=0, y=3)


frame_alpr = tk.LabelFrame(root, text=" --Process-- ", width=220, height=350, bd=5, font=('times', 14, ' bold '),bg="black",fg='white')
frame_alpr.grid(row=0, column=0, sticky='nw')
frame_alpr.place(x=10, y=190)
...
#    global fn
    ...
    if fn!="":
        ...
        if plant == 0:
            Cd="No Retina Detected \n Precautions: Maintain good blood sugar control through diet, \n exercise, and medication "
        elif plant == 1:
            Cd="Mild Retina Detected\n Precautions:  Maintain a healthy weight,\nControl your blood pressure and cholesterol "
            #print('Retinal Disease Detected')
            
            # sms_send()
        elif plant == 2:
            Cd="Moderate Retina Detected \n Precautions: Laser treatment to seal leaking \nblood vessels and prevent further damage,\nMedications to reduce swelling and inflammation"
            sms_send()
        A=Cd
        return A
...
def update_label(str_T):
    #clear_img()
    result_label = tk.Label(root, text=str_T, width=50, font=("bold", 25), bg='#DA70D6', fg='black')
...
def test_model():
    ...
    if fn!="":
        ...
        msg="Image Browsing Completed.."+'\n'+ x2 + '\n'+ ET
        fn=""
    else:
        msg="Please Select Image For Detection...."
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='C:/Users/sachi/OneDrive/Desktop/BEPROJECT/Retinal Diseases Detection/test_set', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
...
button1 = tk.Button(frame_alpr, text=" Browse Image ", command=openimage,width=15, height=1, font=('times', 15, ' bold '),bg="white",fg="black")
button1.place(x=10, y=40)

button2 = tk.Button(frame_alpr, text="Image Preprocessing", command=convert_grey, width=15, height=1, font=('times', 15, ' bold '),bg="white",fg="black")
...
# exit1 = tk.Button(frame_alpr, text="Accuracy Graph", command=window1, width=15, height=1, font=('times', 15, ' bold '),bg="white",fg="black")
